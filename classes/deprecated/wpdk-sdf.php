 ...
 *
 *     $fields = array(
 *        'Fieldset Legend name' => array(
 *             'Optionale subtitle',                       // Optional subtitle
...
class WPDKSDF {
    ...
    /**
     * @var array Easy Array Form
     */
    public $fields;

    /**
     * @var bool TRUE for enable legend fieldset in accordion mode. Default is FALSE
     */
    public $accordion;

    /**
     * @var int Usato per creare delle sezioni con id che si incrementa
     */
    private $section_id;
    ...
    function html() {
        ...
        foreach ( $this->fields as $legend_section => $rows ) {

            /* Check for accordion mode. */
            $accordion_open = ( empty( $accordion_class ) ) ? '' : ( ( 'wpdk-accordion-open' != $accordion_open ) ? 'wpdk-accordion-open' : '' );

            $class = 'wpdk-form-fieldset wpdk-form-section' . $this->section_id;
            $class = apply_filters( 'wpdk_form_section_class', $class, $this->section_id++, $legend_section ); ?>

            <fieldset class="<?php echo $class ?> <?php echo $accordion_open ?>">
                <legend><?php echo $legend_section ?><?php echo $open_close ?></legend>
                <div class="wpdk-fieldset-container <?php echo $accordion_class ?> <?php echo $accordion_open ?>">
                    <div>
                        <?php $this->process_rows( $rows ) ?>
                    </div>
                </div>
            </fieldset>
            <?php
        ...
        foreach ( $rows as $item ) {
            if ( is_string( $item ) && !empty( $item ) ) {
                ...
                if ( isset( $item['type'] ) ) {
                    ...
                    $this->process_rows( $item['group_inner'] );
